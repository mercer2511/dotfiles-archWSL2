...
main() {
  ...
  local date_time
  date_time="$(get_tmux_option "@catppuccin_date_time" " %d/%m/%Y %H:%M")"
  readonly date_time

  local git_status
  git_status="$(get_tmux_option "@catppuccin_git" "off")"
  readonly git_status
  ...
  readonly show_date_time="#[fg=$thm_blue,bg=$thm_gray]$right_separator#[fg=$thm_bg,bg=$thm_blue]󰃭 #[fg=$thm_fg,bg=$thm_gray] $date_time "

  local show_git
  readonly show_git="#[fg=$thm_blue,bg=$thm_gray]$right_separator#[fg=$thm_bg,bg=$thm_blue]󰘬 #[fg=$thm_fg,bg=$thm_gray] #(cd #{pane_current_path}; git branch 2>/dev/null | grep '*' | cut -d' ' -f2-) "
  ...
  if [[ "${git_status}" == "on" ]]; then
    right_column2=$right_column2$show_git
  fi
